...
void CCharacter9Map::MoveCharacter(void)
{
 ...
 for(int n=0;n<CharacterAmount;n++)
 {
  ...
  if (sCharacter9[n].TimeOfLife==0)//время жизни персонажа закончилось
  {
   DestroyCharacter(n);
   continue;
  }
...
bool CCharacter9Map::ImpactCharacterToMissile(int xleft,int xright,int yup,int ydown)
{
 ...
 {
  if (sCharacter9[n].LifeMode!=2) continue;
  int pd_face1=CircleIntercectionToLine((float)xleft,(float)yup,(float)xright,(float)yup,sCharacter9[n].X,sCharacter9[n].Y,(float)Radius);//верхняя
  int pd_face2=CircleIntercectionToLine((float)xleft,(float)ydown,(float)xright,(float)ydown,sCharacter9[n].X,sCharacter9[n].Y,(float)Radius);//нижняя
  int pd_face3=CircleIntercectionToLine((float)xleft,(float)yup,(float)xleft,(float)ydown,sCharacter9[n].X,sCharacter9[n].Y,(float)Radius);//левая
  int pd_face4=CircleIntercectionToLine((float)xright,(float)yup,(float)xright,(float)ydown,sCharacter9[n].X,sCharacter9[n].Y,(float)Radius);//правая
